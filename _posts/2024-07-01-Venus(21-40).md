...
categories: [Training platform,Hackmyvm,Venus]  
tags: [Venus]  
permalink: "/Hackmyvm/Venus(21-40).html"
---
...
iris@venus:~$
```
发现似乎是一个照片，尝试进行识别，使用`cyberchef`，base64解码以后点一下魔术棒得到密码：
```text
yOUJlV0SHOnbSPm
```
...
```
找到一个flag，名为`haha`的flag => `8===xKmPDsJSKpHLzkqKXyjx===D~~`
...
```
这里下载到本地，我是用的termius，然后`SFTP`传过来的。
